...
const AUDIO_EXT_RE = /\.(mp3|mpeg|mpga|m4a|wav|ogg|aac|opus)$/i;
...
function looksLikeAudioFile(fileOrName, mime = "") {
  const name = typeof fileOrName === "string" ? fileOrName : String(fileOrName?.name || "");
  const type = typeof fileOrName === "string" ? String(mime || "") : String(fileOrName?.type || mime || "");
  return /^audio\//i.test(type) || AUDIO_EXT_RE.test(name) || /\.mp3\.mpeg$/i.test(name);
}
...
async function sbUpload(file) {
  ...
  const fallbackType = mimeFromFileName(safe);
  const esAudio = looksLikeAudioFile(safe, file.type || fallbackType);
  const contentType = esAudio
    ? (/^audio\//i.test(file.type || "") ? file.type : /^audio\//i.test(fallbackType) ? fallbackType : "audio/mpeg")
...
function RouteEditor({ data, persist, busy }) {
  ...
  const uploadAudio = async (bi, file) => {
    if (!file) return;
    const looksAudio = looksLikeAudioFile(file);
        ...
        <div key={b.id} className={`card block-edit ${b.locked ? "block-edit--locked" : ""}`}>
          ...
          <div className="ppt-input-group">
            <label className="btn btn--teal-o btn--sm ppt-upload-btn">
              ...
              <input type="file" accept=".mp3,.mpeg,.mpga,.m4a,.wav,.ogg,.aac,.opus,audio/mpeg,audio/mp4,audio/wav,audio/ogg,audio/aac,audio/*" style={{ display: "none" }} disabled={uploadingAudio === bi} onChange={(e) => { if (e.target.files[0]) uploadAudio(bi, e.target.files[0]); e.target.value = ""; }} />
            </label>
          ...
          <div key={r.id} className="role-resource-row">
            ...
            {roleKey === "coordinador" && (
              <label className="btn btn--teal-o btn--sm ppt-upload-btn">
                ...
                <input
                  type="file"
                  accept={r.type === "audio" ? ".mp3,.mpeg,.mpga,.m4a,.wav,.ogg,.aac,.opus,audio/mpeg,audio/mp4,audio/wav,audio/ogg,audio/aac,audio/*" : "application/pdf,.pdf"}
                  style={{ display: "none" }}
...
function RoleAudioPlayer({ resource, roleKey, onUse }) {
  ...
  const downloadExt = (rawUrl.match(/\.(mp3|mpeg|mpga|m4a|wav|ogg|aac|opus)(?:[?#]|$)/i)?.[1] || "mp3").toLowerCase();
